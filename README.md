...
Following that, the other graphs helped us analyze the prevalence of TB cases, number of TB deaths in all forms, incidence of TB cases including and excluding the effect of HIV, and how the prevalence of TB has evolved over time. When visualized together, these graphs aided us in creating regression models to analyze the correlation between the detection rate and number of deaths. Model 1 included the effect of HIV, while Model 2 excluded it. Both models showed a significant p-value (very low), indicating a negative correlation between the detection rate and number of deaths (as the detection rate increases, the number of deaths decreases). Also, it is important to note that even though our model had a low r-squared value, this doesn't necessarily mean that the model is not significant, but, in contrary, opens the doors to include other features in the model as there might be other factors beyond the independent variable (detection rate) that are influencing the dependent variable (number of deaths). So, in that case, I believe that also the type and accuracy of the detection methods, variable to be analyzed in the future, will provide a better fit to our model.
...
Tuberculosis (TB) remains a widespread issue, with several well-known contributing factors, such as a lack of access to adequate healthcare and treatment in low-income countries and regions, weak immune systems due to comorbidities, lifestyle factors, and age groups. While we were not able to investigate these factors in detail at this project (they weren't part of our dataset so I am mentioning them from different studies done in the field), our study emphasized the importance of detecting and preventing TB. 

Thus, this project's aim was to examine the correlation between the rate of TB detection and the decline in the number of deaths and it achieved that aim by analyzing several graphs and visualizations. Our regression model showed a negative correlation between the increase in detection rate and decrease in the number of TB deaths, with countries in Europe serving as a prime example. In addition of describing a negative correlation between the rate of detection and the number of deaths, it points out to the importance of detecting and preventing measures in first place.
...
- When trying to match significant map and other graphs, the legend and filter mismatch caused the need to explore each graph on separate sheets everytime the dashboard was updated. This also happened with the legend or our regions were sorted manually, but changed automatically by Tableau after;
